        ...
        <div class="box border blue" id="messenger">
            <div class="box-body">
                <div class="table-responsive">
                    <?php
                    $query = "SELECT COUNT(*) AS completed_schemes FROM scheme_lists WHERE scheme_status IN ($scheme_status)";
                        ...
                        <tbody>
                            ...
                            foreach ($districts as $district) { ?>
                                ...
                                foreach ($constituencies as $constituency) { ?>
                                    <tr>
                                        <td></td>
                                        <th style="text-align: right;"><?php echo htmlspecialchars($constituency->na); ?></th>
                                        <?php foreach ($categories as $category) {
                                            $query = "SELECT COUNT(*) AS total FROM schemes 
                                                      WHERE scheme_status IN ($scheme_status) 
                                                      AND district_id = ? 
                                                      AND component_category_id = ? 
                                                      AND na = ?";
                                            $total = $this->db->query($query, [$district->district_id, $category->component_category_id, $constituency->na])->row()->total;
                                        ?>
                                        ...
                                        <th style="text-align: center; background-color:rgb(235, 235, 235);">
                                            ...
                                            $query = "SELECT COUNT(*) AS total FROM schemes 
                                                  ...
                                                  AND component_category_id IN (1,2,3,4,5,6,7,8,9)
                                                  AND na = ?";
                                            echo $total = $this->db->query($query, [$district->district_id, $constituency->na])->row()->total;
                                            ?>
                                        </th>
                                        <th style="text-align: center; background-color: rgb(235, 235, 235);">
                                            <?php $query = "SELECT COUNT(*) AS total FROM schemes 
